...
class PostgresLoader:
    ...
    def _validate_setup(self) -> None:
        ...
        for rule in self.processing_rules:
            ...
            if not mapping_filepath.exists():
                ...
                # ENHANCED: Find a sample file that MATCHES THE RULE'S PATTERN
                sample_file = self._find_matching_sample_file(rule)
                if sample_file:
                    ...
                    logger.error(f"No matching sample file found for {rule.base_name} in {rule.directory} with pattern '{rule.file_pattern}' to generate mapping template")
    ...
    def _find_matching_sample_file(self, rule: FileProcessingRule) -> Optional[Path]:
        """
        Find a sample file that matches the rule's pattern for generating mapping templates.
        Only considers files that match the pattern, respecting skip configurations.
        """
        rule_source_dir = Path(rule.directory)
        if not rule_source_dir.exists():
            return None

        logger.info(f"Looking for sample file matching pattern '{rule.file_pattern}' in {rule.directory}")
        
        files_checked = 0
        matching_files = []
        
        # Search for files based on rule configuration
        ...
        for file_path in search_pattern:
            ...
            # Skip files in processing directories
            ...
            if rule.skip_subdirectories:
                try:
                    ...
                    # If relative path can't be computed, skip
                    continue

            # Check if file matches any skip patterns
            if rule.skip_file_patterns:
                skip_file = False
                for skip_pattern in rule.skip_file_patterns:
                    try:
            ...
            # FINALLY: Check if file matches the rule's pattern
            match = rule.match(filename)
            if match:
                matching_files.append(file_path)
                logger.info(f"Found matching sample file: {file_path} for rule '{rule.base_name}'")

        logger.info(f"Checked {files_checked} files, found {len(matching_files)} matching pattern '{rule.file_pattern}'")
