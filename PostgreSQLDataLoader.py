...
FAILED_ROWS_DIR = "failed_rows"  # NEW
FAILED_ROWS_TO_PROCESS_DIR = os.path.join(FAILED_ROWS_DIR, "to_process")  # NEW
FAILED_ROWS_PROCESSED_DIR = os.path.join(FAILED_ROWS_DIR, "processed")  # NEW
...
METADATA_COLUMNS = {
    ...
    "_GUIDANCE",
    "_source_sheet",
    "_error_message",    # NEW: For failed rows
    "_failed_reason",    # NEW: For failed rows
    "_source_file",      # NEW: For tracking original file
    "_export_type"       # NEW: For tracking export type
}
...
class ProcessingConfig:
    ...
    # NEW: Enhanced insertion settings
    enable_row_level_recovery: bool = True
    fail_on_partial_insert: bool = False  # If True, fails when any rows fail
    retry_on_deadlock: bool = True
    max_retry_delay: int = 30  # Maximum seconds to wait between retries
    enable_batch_validation: bool = True  # Pre-validate data before insert
    chunk_size: int = 100  # Process rows in chunks
    max_chunk_failures: int = 5  # Stop if too many chunks fail
...
class FileContext:
    ...
    is_failed_row: bool = False  # NEW: For failed rows processing
    source_sheet: str = ""
...
class PostgresLoader:
    ...
    def _create_directory_structure(self):
        """Create the organized directory structure with failed_rows."""
    ...
    # NEW: Failed rows export with consistent naming
    def _export_failed_rows(self, original_df: pd.DataFrame, error_details: List[Dict], 
                           file_context: FileContext, target_table: str):
        """
        Export failed rows using ORIGINAL filename for consistency with duplicates/conflicts.
        """
        ...
        # Add error metadata columns (will be auto-removed during reprocessing)
        ...
        # Add guidance column - consistent with duplicates/conflicts
        ...
        failed_df.to_csv(export_path, index=False)
    ...
    # ---------------------------
    # Files discovery - UPDATED with failed_rows
    # ---------------------------
    def get_files_to_process(self) -> List[FileContext]:
        ...
        # NEW: failed_rows/to_process
        failed_rows_to_process_dir = Path(FAILED_ROWS_TO_PROCESS_DIR)
        if failed_rows_to_process_dir.exists():
    ...
    # ---------------------------
    # Processing loop - UPDATED with failed_rows
    # ---------------------------
    ...
    def process_file(self, file_context: FileContext) -> bool:
        try:
            ...
            if success:
                ...
                elif getattr(file_context, 'is_failed_row', False):
                    self._move_to_processed(file_context.filepath, "failed_rows")

            # Optionally delete file after successful processing
    ...
    def _process_dataframe(self, df: pd.DataFrame, file_context: FileContext) -> bool:
        ...
        # Add system columns
        insert_df['loaded_timestamp'] = datetime.now()
